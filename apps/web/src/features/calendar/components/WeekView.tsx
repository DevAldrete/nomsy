...
export function WeekView() {
  ...
  // Build week days from selectedWeekMonday (local) so each row matches the correct calendar day.
  // Using weekStart string parsed as UTC caused entries to appear on the wrong day in some timezones.
  const weekDays = (() => {
    return Array.from({ length: 7 }, (_, i) => {
      const d = new Date(selectedWeekMonday);
      d.setDate(selectedWeekMonday.getDate() + i);
      return d;
    });
  })();
