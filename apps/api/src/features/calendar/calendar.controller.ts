...
export async function add(req: Request, res: Response): Promise<void> {
  ...
  // Parse YYYY-MM-DD as noon UTC so the calendar day is correct in all timezones.
  // Otherwise new Date("2026-03-13") is UTC midnight, which is the previous day in e.g. Americas.
  const dateObj =
    /^\d{4}-\d{2}-\d{2}$/.test(date) ? new Date(`${date}T12:00:00.000Z`) : new Date(date);
